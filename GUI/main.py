...
root.iconbitmap(r'C:\Users\amans\Desktop\aman\projects\DAS\Driver-Alertness-System\GUI\images\icon.ico')
...
def model_test():
    ...
    result_loss["text"] = str(f"Loss  :  {str(result[0])}")
    result_accuracy["text"] = str(f"Accuracy  :  {str(result[1])}")
    frame_result.update()
...
def model_train():
    ...
    model.save('GUI/models/model1.h5', overwrite=True)

    time_taken = tm.time() - train_initial_time

    label_time_taken["text"] = str(f"Time Taken :  {str(time_taken)} Seconds")
    label_time_taken.update()
...
def cam0():  # sourcery skip: assign-if-exp, low-code-quality
    face = cv.CascadeClassifier("GUI/cascadeClassifier/haarcascade_face.xml")
    eye = cv.CascadeClassifier("GUI/cascadeClassifier/haarcascade_eye.xml")

    model = load_model("GUI/models/eyestatus.h5")
    ...
    mixer.init()
    mixer.music.load("GUI/audio/alert.ogg")
...
def cam1():  # sourcery skip: assign-if-exp, use-fstring-for-concatenation
    face = cv.CascadeClassifier("GUI/cascadeClassifier/haarcascade_face.xml")
    eye = cv.CascadeClassifier("GUI/cascadeClassifier/haarcascade_eye.xml")
...
img_train = cv.imread("GUI/images/img_train.png")
...
img_test = cv.imread("GUI/images/img_test.png")
...
img_main = cv.imread("GUI/images/img_main.png")
...
img_browse = cv.imread("GUI/images/img_browse.png")
...
img_start = cv.imread("GUI/images/img_start.png")
...
img_browse1 = cv.imread("GUI/images/img_browse.png")
...
img_browse2 = cv.imread("GUI/images/img_browse.png")
...
img_start1 = cv.imread("GUI/images/img_start.png")
...
img_camOFF = ImageTk.PhotoImage(Image.open('GUI/images/img_camOFF.png'))
